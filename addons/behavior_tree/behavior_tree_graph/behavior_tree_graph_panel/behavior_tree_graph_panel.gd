...
func set_behavior_tree(to : BehaviorTree) -> void:
	behavior_tree = to
	_clear_navigation_buttons()
	_add_navigation_button("Main")
	set_graph("Main")


func set_graph(to : String) -> void:
	graph = to
	...
	_add_nodes(behavior_tree.graphs[graph])


func save_graph() -> void:
	behavior_tree.graphs[graph] = _save_nodes(graph_edit.get_children())
	behavior_tree.remove_unused_graphs()


func _add_nodes(nodes : Array, select := false, offset := Vector2()) -> void:
	var graph_nodes := {}
	for node in BehaviorTree.get_flat_nodes(nodes):
		var new_node := BehaviorNode.instance()
		graph_edit.add_child(new_node)
		new_node.connect("close_request", self,
				"_on_BehaviorNode_close_request", [new_node])
		new_node.connect("offset_changed", self,
				"_on_BehaviorNode_offset_changed", [new_node])
		new_node.connect("resize_request", self,
				"_on_BehaviorNode_resize_request", [new_node])
		new_node.connect("group_edited", self, "_on_BehaviorNode_group_edited")
		new_node.connect("group_name_changed", self,
				"_on_BehaviorNode_group_name_changed")
		new_node.init(node)
...
func _save_nodes(nodes : Array) -> Array:
	var data := {}
	for node in nodes:
...
func _on_GraphEdit_duplicate_nodes_request() -> void:
	...
	for node in graph_edit.get_children():
		if node is GraphNode:
			...
			node.selected = false
	var nodes := _save_nodes(to_duplicate)
	_add_nodes(nodes, true, Vector2.ONE * 30)
...
func _on_GraphEdit_paste_nodes_request() -> void:
	...
	for node in graph_edit.get_children():
		if node is GraphNode:
			node.selected = false
	_add_nodes(copied, true, graph_edit.get_local_mouse_position()\
...
func _show_create_dialog(at_mouse := false) -> void:
...
func _on_CreateBehaviorNodeDialog_hide() -> void:
	from_node = ""
	to_node = ""
...
func _on_BehaviorNode_close_request(node : GraphNode) -> void:
	node.queue_free()
	save_graph()
	set_graph(graph)


func _on_BehaviorNode_offset_changed(node : GraphNode) -> void:
	node.data.position = node.offset


func _on_BehaviorNode_resize_request(new_minsize : Vector2,
		node : GraphNode) -> void:
	node.rect_size = new_minsize
	node.data.size = new_minsize


func _on_BehaviorNode_group_edited(group : String) -> void:
	_add_navigation_button(group)
	save_graph()
	set_graph(group)


func _on_BehaviorNode_group_name_changed(from : String, to : String) -> void:
...
func _add_navigation_button(group : String) -> void:
...
func _clear_navigation_buttons() -> void:
